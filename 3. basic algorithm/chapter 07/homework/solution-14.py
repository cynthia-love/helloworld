...
"""
    对PositionLinkedList实现find(e)
    要求用递归而非循环

    分析空间复杂度

    return p if p.node.e == e else find(self.after(p))

    这里不需要保存父函数的活动记录, 即尾递归
    如果编译器有自动优化功能, 空间复杂度为O(1), 否则为O(n)
...
class PositionLinkedList(DoubleLinkedList):
    ...
    class Position:
        ...
        def __eq__(self, other):
            ...
            if not other.container is self.container:
                return False

            if not other.node is self.node:
                return False
    ...
    def find(self, e):
        ...
        def rf(p, e):
            ...
            if p.element == e:
                ...
                return rf(self.after(p), e)

        return rf(self.first(), e)
    # ---mutator---
